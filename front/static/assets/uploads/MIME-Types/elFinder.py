...
class connector():
  ...
  # 'aclRole': 'user', # TODO
    ...
    _mimeType = {
        # text
        'txt': 'text/plain', 'php': 'text/x-php', 'html': 'text/html', 'htm':
        'text/html', 'js': 'text/javascript', 'css': 'text/css', 'rtf':
        'text/rtf', 'rtfd': 'text/rtfd', 'py': 'text/x-python', 'java':
        'text/x-java-source', 'rb': 'text/x-ruby', 'sh': 'text/x-shellscript',
        'pl': 'text/x-perl', 'sql': 'text/x-sql',  # apps
        'doc': 'application/msword', 'ogg': 'application/ogg', '7z':
        'application/x-7z-compressed',  # video
        'ogm': 'appllication/ogm', 'mkv': 'video/x-matroska'
    }
    ...
    def __init__(self, opts):
        ...
        if self._options['debug']:
            ...
            self._today = time.mktime(datetime(t.year, t.month,
                                               t.day).timetuple())
    ...
    def run(self):
        ...
        if not os.path.exists(
            self._options['root']) or self._options['root'] == '':
            rootOk = False
        ...
        possible_fields = ['cmd', 'target', 'targets[]', 'current', 'tree',
        ...
        if ('cmd' in self._request and self._request['cmd'] ==
            'upload') or self._options['debug']:
    ...
    def __open(self):
        ...
        if 'current' in self._request:
            ...
            if not self.__isAllowed(curDir, 'read') or not self.__isAllowed(
                curFile, 'read'):
            ...
            if os.path.islink(curFile):
                ...
                if not curFile or os.path.isdir(curFile):
                    print 'HTTP/1.x 404 Not Found\n\n'
                    sys.exit('File not found')
                if (not self.__isAllowed(os.path.dirname(curFile), 'read') or
                    not self.__isAllowed(curFile, 'read')):
                    print 'HTTP/1.x 403 Access Denied\n\n'
                    sys.exit('Access denied')
            ...
            print 'Content-Disposition: ' + disp + '; filename=' + os.path.basename(
                curFile)
    ...
    def __rename(self):
        ...
        elif not self.__isAllowed(curDir,
                                  'write') and self.__isAllowed(curName, 'rm'):
            self._response['error'] = 'Access denied'
    ...
    def __paste(self):
        if 'current' in self._request and 'src' in self._request and 'dst' in self._request:
            ...
            if not self.__isAllowed(src, 'read') or not self.__isAllowed(
                dst, 'write'):
                self._response['error'] = 'Access denied'
                return
    ...
    def __duplicate(self):
        if 'current' in self._request and 'target' in self._request:
            ...
            if not curDir or not target:
                self._response['error'] = 'Invalid parameters'
                return
            if not self.__isAllowed(target, 'read') or not self.__isAllowed(
                curDir, 'write'):
                self._response['error'] = 'Access denied'
    ...
    def __resize(self):
        if not ('current' in self._request and 'target' in self._request and
                'width' in self._request and 'height' in self._request):
            self._response['error'] = 'Invalid parameters'
            return
    ...
    def __info(self, path):
        ...
        info = {
            ...
            'hash': self.__hash(path),
            'mime': 'directory'
            if filetype == 'dir' else self.__mimetype(path),
            'date': fdate,
            'size': self.__dirSize(path)
            if filetype == 'dir' else stat.st_size,
    ...
    def __tree(self, path):
        ...
        if self.__isAllowed(path, 'read'):
            for d in sorted(os.listdir(path)):
                ...
                if os.path.isdir(pd) and not os.path.islink(
                    pd) and self.__isAccepted(d):
    ...
    def __uniqueName(self, path, copy=' copy'):
        ...
        if not os.path.isdir(path) and re.search(r'\..{3}\.(gz|bz|bz2)$',
                                                 curName):
            pos = -7
            if curName[-1:] == '2':
                pos -= 1
    ...
    def __archive(self):
        ...
        if (not self._options['archivers']['create'] or
            not 'type' in self._request or not 'current' in self._request or
            not 'targets[]' in self._request or not 'name' in self._request):
            self._response['error'] = 'Invalid parameters'
            return

        curDir = self.__findDir(self._request['current'], None)
        archiveType = self._request['type']
        if (not archiveType in self._options['archivers']['create'] or
            not archiveType in self._options['archiveMimes'] or not curDir):
    ...
    def __isAllowed(self, path, access):
        ...
        for ppath in self._options['perms']:
            regex = r'' + ppath
            if re.search(regex,
                         path) and access in self._options['perms'][ppath]:
